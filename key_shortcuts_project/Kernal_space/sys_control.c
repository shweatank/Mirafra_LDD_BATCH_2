...
#define LOG_FILE "/var/log/sys_control.log"
...
static void write_log(const char *message) {
    ...
    snprintf(log_buf, sizeof(log_buf), "[%lld.%09ld] %s\n",
             (long long)ts.tv_sec, ts.tv_nsec, message);

    filp = filp_open(LOG_FILE, O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (IS_ERR(filp)) {
        printk(KERN_ERR "Failed to open log file: %ld\n", PTR_ERR(filp));
        return;
    }

    kernel_write(filp, log_buf, strlen(log_buf), &pos);
...
static void update_feedback(void) {
    ...
    case 0: // Brightness
        ...
        for (i = 0; i < 10; i++)
            buf[i] = i < bars ? '#' : ' ';
        buf += 10;
        snprintf(buf, MAX_BUFFER - strlen("Brightness: [] "), "] %d%% (by Shubham)\n", percentage);
        break;
    case 1: // Volume
        ...
        for (i = 0; i < 10; i++)
            buf[i] = i < bars ? '#' : ' ';
        buf += 10;
        snprintf(buf, MAX_BUFFER - strlen("Volume: [] "), "] %d%% (by Akash R)\n", percentage);
        break;
    case 2: // CPU Governor
        snprintf(buf, MAX_BUFFER, "CPU Governor: %s (by Aravinth)\n",
...
static void write_volume_cmd(const char *cmd) {
    ...
    snprintf(log_buf, sizeof(log_buf), "Volume %s to %d%% (by Akash R)", cmd, control_state.volume);
    write_log(log_buf);
...
static void kbd_handler(int keycode, int value, void *data) {
    ...
    if (control_state.alt_pressed) {
        ...
        case KEY_UP:
            if (control_state.mode == 0) { // Brightness
                if (control_state.brightness < control_state.max_brightness) {
                    ...
                    snprintf(log_buf, sizeof(log_buf), "Brightness increased to %d (by Shubham)",
        ...
        case KEY_DOWN:
            if (control_state.mode == 0) { // Brightness
                if (control_state.brightness > 0) {
                    ...
                    write_sysfs_int(BRIGHTNESS_PATH, control_state.brightness);
                    snprintf(log_buf, sizeof(log_buf), "Brightness decreased to %d (by Shubham)",
                             control_state.brightness);
                    write_log(log_buf);
                }
            } else if (control_state.mode == 1) { // Volume
        ...
        snprintf(log_buf, sizeof(log_buf), "Switched to mode %d (%s)",
                 control_state.mode,
                 control_state.mode == 0 ? "Brightness (by Shubham)" :
                 control_state.mode == 1 ? "Volume (by Akash R)" :
                 "CPU Governor (by Aravinth)");
        write_log(log_buf);
        if (control_state.mode == 2) {
            // Toggle CPU governor
            const char *governor = control_state.brightness > control_state.max_brightness / 2 ?
                                   "performance" : "powersave";
            write_sysfs_str(CPU_GOVERNOR_PATH, governor);
            snprintf(log_buf, sizeof(log_buf), "CPU Governor set to %s (by Aravinth)", governor);
            write_log(log_buf);
        }
    }
...
static int __init sys_control_init(void) {
    ...
    update_feedback();
    write_log("System Control initialized (by Shubham, Akash R, Aravinth)");
...
static void __exit sys_control_exit(void) {
    ...
    write_log("System Control unloaded (by Shubham, Akash R, Aravinth)");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Shubham, Akash R, Aravinth");
MODULE_DESCRIPTION("Keyboard-driven system control with ASCII feedback and logging");
