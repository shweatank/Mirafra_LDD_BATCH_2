...
static unsigned int key_press_count = 0; // Global counter

// Interrupt handler function
static irqreturn_t keyboard_interrupt_handler(int irq, void *dev_id)
{
    key_press_count++; // Increment counter
    printk(KERN_INFO "Keyboard interrupt received! Total key presses: %u\n", key_press_count);
    return IRQ_HANDLED;
}
...
static void __exit my_exit(void)
{
    ...
    printk(KERN_INFO "Total key presses during module lifetime: %u\n", key_press_count);
}
...
MODULE_DESCRIPTION("Keyboard Interrupt Handler with Key Press Counter");
