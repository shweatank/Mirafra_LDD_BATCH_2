...
static int intel_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    bar_start = pci_resource_start(pdev, 0);//phys add
    bar_len   = pci_resource_len(pdev, 0);//size
...
static void intel_remove(struct pci_dev *pdev)
{
    ...
    if (hw_addr)
        iounmap(hw_addr);//unmapping the virtual address
    pci_release_region(pdev, 0);//release the BAR region
    pci_disable_device(pdev);//diable the device
}
