...
#define UART_CR_UARTEN (1 << 0)  /* UART Enable */ //bit pos 4 status flag in the  uart flag register
...
static struct file_operations rpi_uart_fops = {
    .owner = THIS_MODULE, //rpi_uart.ko
    .open = rpi_uart_open,
    .release = rpi_uart_release,
    .read = rpi_uart_read,
    .write = rpi_uart_write,
};//rpi_uart_fops are file operation struct in the kernel module
...
MODULE_DEVICE_TABLE(of, rpi_uart_of_match);//to make it availble to user sopace like module is loaded or not using modinfo
...
{
    return platform_driver_register(&rpi_uart_driver); //register driver with kernel
}

static void __exit rpi_uart_module_exit(void)
{
    platform_driver_unregister(&rpi_uart_driver);//removing or unregister the driver from hardware
}
