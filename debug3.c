...
Sequence* create_sequence(int size) {
    ...
    seq->size = size;
    for (int i = 0; i < size; i++) //fix acessing out of array bounds
        seq->data[i] = i + 1;
    return seq;
}

void apply_transform(Sequence *seq) {
    for (int i = 0; i < seq->size; i++) //fix acessing out of array bounds
...
int main() {
    ...
    apply_transform(seq);
    print_sequence(seq);
     destroy_sequence(&seq); //calling clean up fn

    // Forgot to destroy memory!
}
