...
static int pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
    ...
    hw_addr = ioremap(bar_start, 128 * 1024);

    u32 ral = read_reg(0x5400);
    ...
    write_reg(REG_CTRL, read_reg(REG_CTRL) | (1 << 26));//	Reads control register & Writes it back — triggers reset
    udelay(10);
    ...
    // Build valid Ethernet frame
    u8 dest_mac[6] = {0x00, 0xbe, 0x43, 0x15, 0xee, 0xcc}; // Set to other machine's MAC
