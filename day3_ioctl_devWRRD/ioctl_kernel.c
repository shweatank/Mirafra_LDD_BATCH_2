...
#define DEVICE_NAME "ioctl_kernel"
...
static int dev_open(struct inode *inodep, struct file *filep) {
    ...
    if (!calc_buffer) {
        pr_err("ioctl_kerenlFailed to allocate memory\n");
        return -ENOMEM;
    }
    pr_info("ioctl _kernelDevice opened\n");
    return 0;
}
...
static int dev_release(struct inode *inodep, struct file *filep) {
    if (calc_buffer) {
        kfree(calc_buffer);
        pr_info("ioctl_kernel Memory freed\n");
    }
    pr_info("ioctl _kernel Device closed\n");
    return 0;
}
...
static int __init simple_calculator_init(void) {
    ...
    if (major < 0) {
        pr_err(" ioctl_kernel Failed to register device\n");
        return major;
    }
    pr_info("ioctl kernel Loaded (Major: %d)\n", major);
    return 0;
}
