...
#define DEVICE_NAME "squre_number"
...
static void perform_calculation(void) {
	pr_info("number=%d\n",calc_buffer->num1);
    	calc_buffer->result=(calc_buffer->num1) * (calc_buffer->num1);
        pr_info("squre of a number=%d\n",calc_buffer->result);
    }


// IOCTL function
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
struct calc_data *buffer = file->private_data;
    switch (cmd) {
        case IOCTL_SET_DATA:
...
static int __init simple_calculator_init(void) {
    ...
    pr_info("squre function Loaded (Major: %d)\n", major);
    return 0;
}
...
static void __exit simple_calculator_exit(void) {
    ...
    pr_info("Squre function Unloaded\n");
}
