...
#define RAMBLOCK_NUM_SECTORS 1024 // 512KB
...
static void ramblock_transfer(struct request *req)
{
    ...
    rq_for_each_segment(bvec, req, iter) {
        ...
        if (rq_data_dir(req) == WRITE)
            memcpy(ramblock_data + sector * RAMBLOCK_SECTOR_SIZE, buffer, sectors * RAMBLOCK_SECTOR_SIZE);
        else
            memcpy(buffer, ramblock_data + sector * RAMBLOCK_SECTOR_SIZE, sectors * RAMBLOCK_SECTOR_SIZE);
...
static int __init ramblock_init(void)
{
    ...
    if (!ramblock_data) {
        printk(KERN_ERR "ram_block: Unable to allocate memory.\n");
        return -ENOMEM;
    }
    ...
    printk(KERN_INFO "ram_block: Module loaded successfully.\n");
    return 0;
}
...
MODULE_DESCRIPTION("RAM-backed Block Device Driver (Linux 6.x Compatible)");
