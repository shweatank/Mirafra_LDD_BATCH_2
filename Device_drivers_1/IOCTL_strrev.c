...
#define DEVICE_NAME "IOCTL_strrev"
...
MODULE_DESCRIPTION("Kernel String Reversal  using IOCTL and kmalloc");
MODULE_VERSION("1.0");
...
// Perform calculation
static void perform_revstr(void) {
    int len,i;
    for(len=0;calc_buffer ->inp_str[len]!='\0';len++)
    for(i=0;i<len;i++)
    {
	    calc_buffer->res_str[i] = calc_buffer->inp_str[len-i-1];
    }
    printk(KERN_INFO "%s\n",calc_buffer->inp_str);
...
static int __init simple_calculator_init(void) {
    ...
    pr_info("Reverse string  Loaded (Major: %d)\n", major);
    return 0;
}
